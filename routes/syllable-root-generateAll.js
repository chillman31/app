...
const { generateLists } = require('../utilitaires/function')

module.exports = (app, routeName) => {
    app.get(`/api/generate/syllable/all`, auth, async (req, res) => {
        try {
            for(const list of routeName) await generateLists(list);  
